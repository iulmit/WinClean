...
internal static class Helpers
{
    ...
    /// <summary>
    /// MAX_PATH ( <c>260</c>) minus the null terminator ( <c>'\0'</c>), the drive letter, it's
    /// colon and slash and a trailing slash.
    /// </summary>
    ...
    /// <param name="culture">
    /// The culture to use. If not specified, <see cref="CultureInfo.CurrentUICulture"/> is used.
    /// </param>
    /// <returns>
    /// The name of the resource <paramref name="resource"/>. If <paramref name="resource"/> is not
    /// found, <see langword="null"/>.
    /// </returns>
    ...
    /// </param>
    /// <returns>
    /// All the resources the resource manager have access to of type <typeparamref name="T"/>.
    /// </returns>
    /// <exception cref="ArgumentException">
    /// The resource set for <paramref name="resourceManager"/> doesn't exist.
    /// </exception>
    ...
    /// <returns>
    /// <see langword="true"/> if <paramref name="filenameCandidate"/> can be a filename, otherwise;
    /// <see langword="false"/>. This method returns <see langword="false"/> if <paramref
    /// name="filenameCandidate"/> is <see langword="null"/>.
    /// </returns>
    ...
    /// </param>
    /// <returns>
    /// <paramref name="filenameCandidate"/>, modified to be a valid Windows filename if it wasn't already.
    /// </returns>
    /// <exception cref="ArgumentException">
    /// <paramref name="filenameCandidate"/> or <paramref name="replaceInvalidCharsWith"/> are <see
    /// langword="null"/>, or <paramref name="replaceInvalidCharsWith"/> contains invalid filename chars.
    /// </exception>
